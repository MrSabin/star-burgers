...
Это сайт сети ресторанов Star Burger. Здесь можно заказать превосходные бургеры с доставкой на дом. Ознакомиться с сайтом можно, перейдя по [ссылке](https://pyprojects.ru)
...
- `DEBUG` — (опционально) дебаг-режим. По умолчанию `False`.
...
При необходимости использования сервиса Rollbar необходимо указать дополнительные настройки:

- `ENABLE_ROLLBAR` - `True` для включения сервиса (по умолчанию выключен)
...
- `ROLLBAR_ENV` - `development` или `production` метка сервера
...
## Запуск сайта в docker-контейнерах

Запуск контейнеров с проектом в docker-compose можно выполнить с помощью команды:

```sh
docker compose up -d
```

либо с помощью скрипта `deploy-docker.sh`:

```sh
./deploy-docker.sh
```

Обе команды выполнять из корня репозитория.
